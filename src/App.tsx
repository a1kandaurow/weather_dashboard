...
function App() {
  ...
  // Определяем день или ночь по иконке погоды
  const isDay = weather?.icon ? weather.icon.endsWith("d") : true;

  // Определяем нужен ли светлый текст в зависимости от фона
  const needsLightText =
    weather &&
    (!isDay ||
      weather.weatherMain.includes("thunderstorm") ||
